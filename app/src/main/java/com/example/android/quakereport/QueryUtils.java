...
/**
 * Created by Divyani on 16-01-2017.
 */
public final class QueryUtils
{

    // A class declared as final means that the class cannot be extended

    /** Sample JSON response for a USGS query */
    // sample_json_response is static and final means its value wont change
    ...
    /**
     * Return a list of {@link EarthQuake_Single} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthQuake_Single> extractEarthquakes()
    {
        // It is a method which a return type as ArrayList and name extractEarthquakes()
        ...
        try
        {
            // First create the root jsonObject
            JSONObject rootobject=new JSONObject(SAMPLE_JSON_RESPONSE);
            // Now get the feature key
            JSONArray featureArray = rootobject.getJSONArray("features");
            // Traverse all the features arry
            for(int i=0;i<featureArray.length();i++)
            {
                // Now get the jsonObject of single earthquake at index i
                JSONObject singleEarthQuake  = featureArray.getJSONObject(i);
                // Now get the properties json object from the single earthquake
                JSONObject propObject = singleEarthQuake.getJSONObject("properties");
                // Now access the all data in String format with keys as mag,place,time
              String  magnitude = propObject.getString("mag");
                String location = propObject.getString("place");
               String time= propObject.getString("time");
                // For each single node of feature/Single earthquake we add it to array List
                EarthQuake_Single earthQuakeS = new EarthQuake_Single(magnitude,location,time);
